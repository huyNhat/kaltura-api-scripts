...
'''
            ...
            #if obj.createdAt >= 1588316400 or obj.lastPlayedAt >= 1588316400 and :
            #if  "migrate" in obj.tags:

if "_assignment" in obj.userId:
    if(obj.lastPlayedAt is not None and obj.plays is not None):
        print(str(obj.id) + "," + str(datetime.datetime.fromtimestamp(obj.createdAt).strftime('%Y-%m-%d %H:%M:%S')) +"," +str(datetime.datetime.fromtimestamp(obj.lastPlayedAt).strftime('%Y-%m-%d %H:%M:%S'))+ ","+str(obj.plays))
    else:
        print(str(obj.id) + ","+ +str(datetime.datetime.fromtimestamp(obj.createdAt).strftime('%Y-%m-%d %H:%M:%S'))+",null, null")
